...
import (
	...
	utils "CRD-chain/evertrust/utils"
	...
	"CRD-chain/common/math"
	"CRD-chain/consensus/ethash"
	"CRD-chain/core"
	"CRD-chain/core/rawdb"
	...
	"CRD-chain/p2p"
	"CRD-chain/params"
	CRDUtil "CRD-chain/CRDcc/util"
	"CRD-chain/rlp"
	"CRD-chain/rpc"
	evertrustTypes "CRD-chain/evertrust/types"
)
...
//
// https://CRD-chain/wiki/Management-APIs#personal_sign
...
//
// https://CRD-chain/wiki/Management-APIs#personal_ecRecover
...
func (s *PublicBlockChainAPI) CheckNodeDetail(ctx context.Context, blockNr rpc.BlockNumber) map[string]*qualification.NodeDetail {
	var number uint64
	if blockNr != 0 {
	...
	log.Info("取出的commit高度", "高度", number)
	nodeDetails, ok := qualification.CommitHeight2NodeDetailSetCache.Get(number, s.b.ChainDb())
	if ok {
		return nodeDetails.NodeMap
	}
	return nil
}
...
func (s *PublicBlockChainAPI) GetTotalAmountOfHypothecationWithAddress(ctx context.Context, address common.Address) (*big.Int, error) {
	...
	contractAddress := utils.HypothecationContract
	keyHash := hypothecationInfoKeyHash(address.Hex())
	hypothInfoByte := stateDB.GetCRDState(contractAddress, keyHash)
	if len(hypothInfoByte) == 0 {
		//可能已调用过退出委员会的接口
		recaContractAddr := utils.RedamptionContract
		recaKeyHash := quiteQuorumInfoKeyHash(address.Hex())
		recaData := stateDB.GetCRDState(recaContractAddr, recaKeyHash)
...
func hypothecationInfoKeyHash(address string) common.Hash {
	return CRDUtil.EthHash([]byte(fmt.Sprintf("%s:%s:%s", conf.CRDKeyFlag, "hypothecationInfo", address)))
}

func (s *PublicBlockChainAPI) GetRunTime(ctx context.Context) string {

	return fmt.Sprintf("Running Time %.2f Hour", time.Since(evertrust.StartTime).Hours())
}

func (s *PublicBlockChainAPI) GetLocalMinerRewardAddress(ctx context.Context) string {
	return evertrust.Config.String("rewardAddress")
}

func (s *PublicBlockChainAPI) GetNodeDisqualifiedReason(ctx context.Context, address common.Address) (reason string, err error) {
	block, err := s.b.GetCurrentCommitBlock(ctx, common.Hash{})
	if err != nil {
		return "", err
	}
	...
	quor, ok := quorum.CommitHeightToConsensusQuorum.Get(currentHeight, s.b.ChainDb())
	if !ok {
		return "", errors.New("no quorum on this height")
	}

	addr := quor.Get(address.Hex())
	if addr != quorum.EmptyAddress {
		return "query node is in quorum no disqualified reason", nil
	}

	nodeDetail := nodeDetails.Get(address.Hex())
	if nodeDetail == nil {
		return "", errors.New("no nodeDetail record of given address")
	}

	return nodeDetail.DisqualifiedReason, nil
}
...
func (s *PublicBlockChainAPI) GetNodeUselessActiveInfoByAddress(ctx context.Context, address common.Address) (info uint64, err error) {
	block, err := s.b.GetCurrentCommitBlock(ctx, common.Hash{})
	if err != nil {
		return 0, err
	}
...
//获取质押成功的地址
func (s *PublicBlockChainAPI) GetHypothecationSuccessfulAddress(ctx context.Context) ([]string, error) {
	block, err := s.b.GetCurrentCommitBlock(ctx, common.Hash{})
	if err != nil {
		return nil, err
	}
...
func (s *PublicBlockChainAPI) GetCommitBlockByNormalBlockHash(ctx context.Context, hash common.Hash) (map[string]interface{}, error) {
	block, err := s.b.GetBlock(ctx, hash)
	if err != nil {
		return nil, err
	}
	blockExtra := evertrustTypes.BlockExtraDecode(block)
	commitNum := blockExtra.CNumber.Uint64()
	commitBlock, err := s.b.GetCommitBlockByNum(ctx, commitNum)
	if commitBlock != nil {
		response, err := s.rpcOutputCommitBlock(commitBlock)
		return response, err
	}
	return nil, err
}
...
func (s *PublicBlockChainAPI) GetCommitBlockByNormalBlockNumber(ctx context.Context, blockNr rpc.BlockNumber) (map[string]interface{}, error) {
...
func (s *PublicBlockChainAPI) GetBlockByNumber(ctx context.Context, blockNr rpc.BlockNumber, fullTx bool) (map[string]interface{}, error) {
	//if err := CheckQuerypoints(ctx); err != nil {
	//	return nil, err
	//}
	block, err := s.b.BlockByNumber(ctx, blockNr)
	if block != nil {
...
func (s *PublicBlockChainAPI) GetNodeStatus(ctx context.Context, address common.Address) (status int, err error) {
	...
	quoru, ok := quorum.CommitHeightToConsensusQuorum.Get(block.NumberU64(), s.b.ChainDb())
	if !ok {
		return NODE_OFF_LINE, errors.New("no quorum on this height")
	}
	...
	case qualification.CantBeMaster:
		...
		_, commitExtra := evertrustTypes.CommitExtraDecode(block)
		...
		data := state.GetCRDState(recaptionAddr, recaptionAddr.Hash())
		if len(data) == 0 {
...
func quiteQuorumInfoKeyHash(address string) common.Hash {
	return CRDUtil.EthHash([]byte(fmt.Sprintf("%s:%s:%s", conf.CRDKeyFlag, "recaption", address)))
}

func (s *PublicBlockChainAPI) GetCommitBlockByHash(ctx context.Context, blockHash common.Hash) (map[string]interface{}, error) {
	block, err := s.b.CommitBlockByHash(ctx, blockHash)
	if block != nil {
		response, err := s.rpcOutputCommitBlock(block)
		return response, err
	}
	return nil, err
}

func (s *PublicBlockChainAPI) GetCommitBlockByNumber(ctx context.Context, blockNr rpc.BlockNumber) (map[string]interface{}, error) {
...
func (s *PublicBlockChainAPI) GetMaxCommitBlockHash(ctx context.Context, chainID string) (commitBlockHah string) {
	...
	keyHash := CRDUtil.EthHash([]byte(key + conf.CRDKeyFlag))
	st := stateDB.GetCRDState(CRDUtil.EthAddress(utils.BaapTrustTree), keyHash)
	if len(st) > 0 {
		maxNum := big.NewInt(0).SetBytes(st)
		log.Info("maxNum", "is", maxNum)

		commitBlockKey := fmt.Sprintf("%s:%s", chainID, maxNum.String())
		commitBlockKeyHash := CRDUtil.EthHash([]byte(commitBlockKey + conf.CRDKeyFlag))
		commitBlockSt := stateDB.GetCRDState(CRDUtil.EthAddress(utils.BaapTrustTree), commitBlockKeyHash)
...
func (s *PublicBlockChainAPI) EstimateGas(ctx context.Context, args CallArgs) (hexutil.Uint64, error) {
	...
	if args.To == nil &&
		...
		// 处理 meta
		code, _, err = evertrust.ParseData(code)
		if err != nil {
			panic(err)
		}
	...
	log.Trace("PublicBlockChainAPI.EstimateGas", "gas", hi, "args", args)
...
func RPCMarshalBlock(b *types.Block, inclTx bool, fullTx bool) (map[string]interface{}, error) {
	...
	blockExtra := evertrustTypes.BlockExtraDecode(b)
	/*
		"extraData": map[string]map[string]interface{}{
				"blockExtra": map[string]interface{}{
					"NodeID":        blockExtra.NodeID.Bytes(),
					"Rank":          blockExtra.Rank,
					"CNumber":       blockExtra.CNumber,
					"Signature":     blockExtra.Signature,
					"IsLandID":      blockExtra.IsLandID,
					"Empty":         blockExtra.Empty,
					"HistoryHeight": (*hexutil.Big)(blockExtra.HistoryHeight),
					"Version":       blockExtra.Version,
					"extraData":     blockExtra.Extra,
				},
			},
	*/
	fields := map[string]interface{}{
		...
		"hash":             b.Hash(),
		"commitNumber":     (*hexutil.Big)(blockExtra.CNumber),
...
func RPCMarshalCommitBlock(b *types.Block) (map[string]interface{}, error) {
	head := b.Header() // copies the header once
	//blockExtra, commitExtra := evertrustTypes.CommitExtraDecode(b)
	/*
		"extraData": map[string]map[string]interface{}{
				"blockExtra": map[string]interface{}{
					"NodeID":        blockExtra.NodeID.Bytes(),
					...
					"extraData": map[string]map[string]interface{}{
						"commitExtra": map[string]interface{}{
							...
							"IslandState":       commitExtra.IslandState,
							"Reset":             commitExtra.Reset,
							"AssertionSum":      (*hexutil.Big)(commitExtra.AssertionSum),
							"QualificationHash": commitExtra.QualificationHash,
							"Version":           commitExtra.Version,
						},
					},
				},
			},
...
func (s *PublicBlockChainAPI) rpcOutputCommitBlock(b *types.Block) (map[string]interface{}, error) {
	fields, err := RPCMarshalCommitBlock(b)
	if err != nil {
		return nil, err
	}
	return fields, err
...
func (s *PublicTransactionPoolAPI) CheckTxCompletedByHash(ctx context.Context, from common.Address, txHash common.Hash) bool {
	...
	keyHash := utils.DepositKeyHash(from, txHash, conf.CRDKeyFlag)
	res := stateDB.GetCRDState(utils.DepositContractAddr(), keyHash)
	if len(res) != 0 {
...
func (s *PublicTransactionPoolAPI) GetUncompletedTransactionByAddress(ctx context.Context, from common.Address) []common.Hash {
	...
	contractAddr := CRDUtil.EthAddress(utils.WithdrawContract)
	keyHash := utils.WithdrawTxKeyHash(from)
	value := stateDB.GetCRDState(contractAddr, keyHash)
	if len(value) == 0 {
		return myTxids
	}
	...
	if proxyServer != "" && iaasServer != "" {
		...
		etherBase := evertrust.Config.String("etherbase")
	...
	for _, v := range txs {
		...
		points := evertrust.Getpoints(privateKey, srcChainid, v.Payload.DstChainID, evertrust.XChainpointsType)
		if points != "" {
			hosts = []string{fmt.Sprintf(evertrust.ProxyRPC, v.Payload.DstChainID)}
		} else {
		...
		//whether has succeed
		db := *ethdb.ChainDb
		completedTxKey := CRDUtil.EthHash([]byte(fmt.Sprintf("evertrust:completed:withdraw:tx:%s", v.Txid)))
...
func (s *PublicTransactionPoolAPI) GetWithDrawTransactionByHash(ctx context.Context, hash common.Hash) (*WithdrawRPCTransaction, error) {
	...
	if tx, blockHash, blockNumber, index := rawdb.ReadTransaction(s.b.ChainDb(), hash); tx != nil {
		...
		var extra evertrustTypes.BlockExtra
		...
		for _, v := range txs {
			...
			if v.Txid == hash.String() {
				withdrawTx.Status = 1
				if res.Cmp(big.NewInt(evertrust.XChainTransferCommitNum)) >= 0 {
					withdrawTx.Status = 2
				}
			}
		}
...
*/
func (s *PublicTransactionPoolAPI) Compilepoints(initialSupply string, name string, symbol string, decimals *big.Int, owner common.Address) (hexutil.Bytes, error) {
	...
	fmt.Println("-------------------------------------------------------------")
	return erc20points.Template.Compile(supply, name, symbol, decimals, owner)
}
...
func (s *PublicTransactionPoolAPI) GetWithDrawTransactionReceipt(ctx context.Context, hash common.Hash) (map[string]interface{}, error) {
	...
	if receipt.Status == types.ReceiptStatusSuccessful {
		//判断是否达到了10*cfd个块的确认
		header, _ := s.b.HeaderByNumber(context.Background(), rpc.LatestBlockNumber) // latest header should always be available
		if header.Number.Uint64()-blockNumber >= evertrust.XChainTransferCommitNum {
...
func (s *PublicTransactionPoolAPI) SendTransaction(ctx context.Context, args SendTxArgs) (common.Hash, error) {
	if !evertrust.Config.Bool("debug") {
...
func (s *PublicTransactionPoolAPI) QueryTrustTreeCommitHash(ctx context.Context, chainID string, commitBlockNumber uint64) []common.Hash {
	stateDB, _, err := s.b.StateAndHeaderByNumber(ctx, rpc.LatestBlockNumber)
	if err != nil {
		log.Error("get state db", "err", err)
		return nil
	...
	commitBlockKeyHash := CRDUtil.EthHash([]byte(commitBlockKey + conf.CRDKeyFlag))
	res := stateDB.GetCRDState(utils.TrustTree, commitBlockKeyHash)
	var cts []common.Hash
	err = json.Unmarshal(res, &cts)
	if err != nil {
		log.Error("unmarshal hash", "err", err)
		return nil
	}
	return cts
}
...
func (s *PublicTransactionPoolAPI) SetCrt(ctx context.Context, jwt string, cert string) (success string, err error) {
	...
	//check cert
	_, _, _, _, _, err = evertrust.ConsortiumSet(cert)
	if err != nil {
		return "", fmt.Errorf("check cert: %v", err)
	}

	//save cert
	localhostCrt := evertrust.DataDir + "/consortium/localhost.crt"
...
func (s *PublicTransactionPoolAPI) GenCsr(ctx context.Context, jwt string, days int, subj string) (csr string, err error) {
	...
	pemFile := evertrust.DataDir + "/node.pem"
...
func (s *PublicTransactionPoolAPI) BaapQuery(ctx context.Context, encodedTx hexutil.Bytes) (string, error) {
	...
	return CRDcc.BaapQuery(stateDB, encodedTx)
}
...
func Filter(tx *types.Transaction) error {
	if tx.GasPrice().Cmp(big.NewInt(0)) <= 0 && !utils.IsFreeGas() {
...
func (s *PublicBlockChainAPI) CallFromTxPool(request utils.PrecreateRequest) *utils.PrecreateResponse {
	...
	stateCRDMap := make(map[common.Address]map[common.Hash][]byte)
	...
	for addr, _ := range state.StateObjects() {
		...
		_stateCRDMap := make(map[common.Hash][]byte)
		// 复制 CRD 状态
		if vm.DeployedContractAccountAddr != addr {
			...
			state.ForEachCRDStorage(addr, func(k common.Hash, v []byte) bool {
				_stateCRDMap[k] = v
		...
		stateCRDMap[addr] = _stateCRDMap
	}
	pendingNumber := s.b.CurrentBlock().Number().Uint64() + 1
	return &utils.PrecreateResponse{pendingNumber, logs, stateMap, stateCRDMap, result, gas, failed, err}
}
