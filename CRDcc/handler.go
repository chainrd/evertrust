...
func (handler *Handler) processStream() error {
	...
	for {
		...
		case sendErr := <-errc:
			if sendErr != nil {
				log.Error("send msg to cc", "err", sendErr)
				return sendErr
			}
			log.Debug("send msg to cc success")
		...
		case in = <-msgAvail:
			...
			log.Debug("Received message from shim", "txid", shorttxid(in.Txid), "type", in.Type.String())
		...
		case nsInfo = <-handler.nextState:
			...
			log.Debug("Move state message","txid",shorttxid(in.Txid), "type", in.Type.String())
		...
		if nsInfo != nil && nsInfo.sendToCC {
			...
			if nsInfo.sendSync {
				...
				log.Debug("serialSendAsync", "txid", shorttxid(in.Txid))
...
func (handler *Handler) sendExecuteMessage(ctxt context.Context,
	...
	log.Debug("into sendExecuteMessage")
	// 如果交易存在，创建将发生err
...
func (handler *Handler) handleGetState(msg *pb.ChaincodeMessage) {
	...
	go func() {
		...
		res := txContext.CRDData.GetRaw(chaincodeID, getState.Key)
...
func (handler *Handler) enterBusyState(e *fsm.Event, state string) {
	...
	go func() {
		...
		if msg.Type.String() == pb.ChaincodeMessage_PUT_STATE.String() {
			...
			log.Info("put state", "chaincodeID", chaincodeID, "msg type", msg.Type.String())
			if conf.NoLimitCC[chaincodeID] == nil {
				num := txContext.CRDData.GetRawNum(chaincodeID)
				stateKeyNum := conf.CCViper.GetInt(conf.SandboxStateKeyNum)
				//log.Info("((((((((((((((((", "num", num, "stateKeyNum", stateKeyNum)
				if num > stateKeyNum && stateKeyNum != -1 {
					log.Error("state num exceeds the maximum", "chaincodeID", chaincodeID, "num", num, "stateKeyNum", stateKeyNum)
				...
				if length > size && size != -1 {
					log.Error("state size(key+value) exceeds the maximum", "maximum", stateSize, "len", length)
			...
			txContext.CRDData.PutRaw(chaincodeID, putState.Key, putState.Value, msg.Txid, msg.Timestamp)
			...
			txContext.CRDData.DelRaw(chaincodeID, delState.Key, msg.Txid, msg.Timestamp)
...
func (handler *Handler) handleQueryStateNext(msg *pb.ChaincodeMessage) {
	go func() {
		...
		if txContext == nil {
			log.Error("transaction context not found (timed out ?)", "txid", msg.Txid)
...
func getRangeResponse(handler *Handler, txContext *transactionContext, iterID string, chaincodeID string, startKey, endKey string) (*pb.QueryResponse, error) {
	...
	res, hasMore := txContext.CRDData.GetRawRange(chaincodeID, startKey, endKey)
...
func getHisResponse(handler *Handler, txContext *transactionContext, iterID string, chaincodeID string, key string, version int) (*pb.QueryResponse, error) {
	...
	res, hasMore := txContext.CRDData.GetRawHis(chaincodeID, key, version)
...
func (handler *Handler) serialSendAsync(msg *pb.ChaincodeMessage, errc chan error) {
	log.Info("handler serialSendAsync", "msg type", msg.Type.String(), "txid", msg.Txid)
	go func() {
...
func (handler *Handler) triggerNextState(msg *pb.ChaincodeMessage, send bool) {
	...
	log.Debug("Handler triggerNextState")
