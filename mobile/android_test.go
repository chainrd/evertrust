...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "evertrust-android-")
	if err != nil {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "evertrust-chain/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "evertrust.aar"), "evertrust.aar", os.ModePerm)
...
dependencies {
    ...
    compile(name: "evertrust", ext: "aar")
}
`
