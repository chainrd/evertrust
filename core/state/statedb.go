...
func (self *StateDB) GetCRDState(a common.Address, b common.Hash) []byte {
	stateObject := self.getStateObject(a)
	if stateObject != nil {
		return stateObject.GetCRDState(self.db, b)
	}
	return []byte{}
}
...
func (self *StateDB) SetCRDState(addr common.Address, key common.Hash, value []byte) {
	// add by liangc : 这个地方如果不加 SetState 的调用 SetCRDState 就不会生效，因为 SetCRDState 没有将 addr 加入 dirty >>>>
	// self.SetState(addr, common.Hash{1}, common.Hash{1})
	// add by liangc : 这个地方如果不加 SetState 的调用 SetCRDState 就不会生效，因为 SetCRDState 没有将 addr 加入 dirty <<<<
	stateObject := self.GetOrNewStateObject(addr)
	if stateObject != nil {
		stateObject.SetCRDState(self.db, key, value)
	}
}
...
func (db *StateDB) ForEachCRDStorage(addr common.Address, cb func(key common.Hash, value []byte) bool) {
	so := db.getStateObject(addr)
	if so == nil {
		return
	}
	...
	// When iterating over the storage check the cache first
	for h, value := range so.CRDCachedStorage {
		cb(h, value)
	}
...
func (self *StateDB) Copy() *StateDB {
	...
	for addr := range self.journal.dirties {
		// As documented [here](https://CRD-chain/pull/16485#issuecomment-380438527),
