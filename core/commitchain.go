...
func NewCommitChain(blockChain *BlockChain) *CommitChain {
	...
	if currentBlock != nil && currentBlock.NumberU64() != 0 && blockChain.CurrentBlock().NumberU64() < commitExtra.NewBlockHeight.Uint64() {
		//根据normal来删除commit
		var cnum int64
		...
		if blockChain.CurrentBlock().NumberU64() == 0 {
			...
			//normal对应的commit高度
			log.Info("从启后要从那个normal开始", "高度", blockChain.CurrentBlock().NumberU64())
			extra := evertrust_types.BlockExtraDecode(blockChain.CurrentBlock())
			cnum = extra.CNumber.Int64()
		}

		for i := int64(currentBlock.NumberU64()); i >= cnum; i-- {
			getBlock = sc.GetBlockByNum(uint64(i))
			DeleteCommitBlock(sc.db, getBlock.Hash(), getBlock.NumberU64(), *sc)
		}
		//保存LastCommitBlock
		if cnum == 0 {
			new(CommitChain).SetupGenesisCommitBlock(blockChain.db)
		} else {
			rawdb.WriteBlock(sc.db, getBlock)
			rawdb.WriteCanonicalCommitHash(sc.db, getBlock.Hash(), getBlock.NumberU64())
			rawdb.WriteHeadCommitBlockHash(sc.db, getBlock.Hash())
			rawdb.WriteHeadCommitHeaderHash(sc.db, getBlock.Hash())
		}
...
func (cc *CommitChain) CleanUpHeightMap(normalBlockHeight uint64) {
	cc.heightmu.Lock()
	defer cc.heightmu.Unlock()
	for h, _ := range cc.blockHeightMap {
		delete(cc.blockHeightMap, h)
	}
}
...
func SetCurrentCommitBlock(sc *CommitChain, block *types.Block) error {
	...
	extra := new(evertrust_types.BlockExtra)
	...
	commitExtra := new(evertrust_types.CommitExtra)
...
func (sc *CommitChain) CurrentBlockExtra() *evertrust_types.BlockExtra {
	return sc.currentBlockExtra.Load().(*evertrust_types.BlockExtra)
}

func (sc *CommitChain) CurrentCommitExtra() *evertrust_types.CommitExtra {
	return sc.currentCommitExtra.Load().(*evertrust_types.CommitExtra)
}
...
func (sc *CommitChain) loadLastState() error {
	...
	if head == (common.Hash{}) {
		var cNum uint64
		normalBlock := sc.blockChain.CurrentBlock()
		if normalBlock.NumberU64() <= 1 {
			cNum = 1
		} else {
			extra := evertrust_types.BlockExtraDecode(normalBlock)
			cNum = extra.CNumber.Uint64()
		}
		commit := sc.GetBlockByNum(cNum)
		if commit != nil {
			head = commit.Hash()
		}
	...
	currentBlock := sc.GetBlockByHash(head)
	//log.Info("loadLastState 的commit区块","currentBlock",currentBlock.Number(),"head",head.String())
	if currentBlock == nil {
		// Corrupt or empty database, init from scratch
...
func (sc *CommitChain) insertChain(block *types.Block) error {
	...
	case CanonStatTy:
		...
		log.Debug("Inserted new commit block", "number", block.Number(), "Rank", blockExtra.Rank, "hash", block.Hash(), "uncles", len(block.Uncles()),
...
func (sc *CommitChain) WriteBlock(block *types.Block) (status WriteStatus, err error) {
	...
	defer sc.mu.Unlock()
	//根据是否是岛屿 判断主链还是侧链
	//commitExtra := evertrust_types.CommitExtraDecode(block)
...
func (sc *CommitChain) SetupGenesisCommitBlock(db ethdb.Database) (common.Hash, error) {
	...
	if (stored == common.Hash{}) {
		commitExtra := evertrust_types.CommitExtra{NewBlockHeight: new(big.Int).SetUint64(0)}
		bytes, _ := commitExtra.Encode()
		blockExtra := evertrust_types.BlockExtra{Rank: 0, CNumber: new(big.Int).SetUint64(0), Extra: bytes}
...
func DeleteCommitBlock(db rawdb.DatabaseDeleter, hash common.Hash, number uint64, commitChain CommitChain) {
	...
	log.Info("DeleteHeader当前commit高度删除完毕", "高度", number)

	rawdb.DeleteBody(db, hash, number)
	log.Info("DeleteBody当前commit高度删除完毕", "高度", number)

	rawdb.DeleteCanonicalCommitHash(db, number)
	log.Info("DeleteCanonicalCommitHash当前commit高度删除完毕", "高度", number)

	rawdb.DeleteHeadCommitBlockHash(db)
	log.Info("DeleteHeadCommitBlockHash当前commit高度删除完毕", "高度", number)

	rawdb.DeleteHeadCommitHeaderHash(db)
	log.Info("DeleteHeadCommitHeaderHash当前commit高度删除完毕", "高度", number)
