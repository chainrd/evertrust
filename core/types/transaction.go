...
var (
	...
	BaapHome      = os.Getenv("CRD_BAAP_HOME")
)
...
func (tx *Transaction) getCertFromBaapChainiaas() (tcAdmCrt string, err error) {
	txP := &protos.Transaction{}
	err = proto.Unmarshal(tx.Data(), txP)
	if err != nil {
		log.Error("getCertFromBaapChainiaas proto unmarshal", "err", err)
		return
	}

	invocation := &protos.Invocation{}
	err = proto.Unmarshal(txP.Payload, invocation)
	if err != nil {
	...
	certB := invocation.Args[1]

	return string(certB), nil
}
...
func (tx *Transaction) VerifyCert(category int, chainId *big.Int) (ok bool) {
	...
	case 4:
		tcAdmCrt, err = tx.getCertFromBaapChainiaas()
	}
	if err != nil {
		return
	}
...
func CheckIAASCert(cert string, tmpCertName string) (ok bool) {
	if BaapHome == "" {
		...
		BaapHome = "/Users/liu/code/CRD-chain/CRDcc"
	}
	conf := BaapHome + "/conf/"
	userCert := conf + tmpCertName
	log.Info("userCert的路径","userCert",userCert)
...
func GenPubKeyFromIAASCert(cert string) (pubKey *ecdsa.PublicKey) {
	...
	if err != nil {
		fmt.Println("UnmarshalPubkey",err)
