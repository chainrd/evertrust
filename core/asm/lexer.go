...
// points is emitted when the lexer has discovered
// a new parsable points. These are delivered over
// the pointss channels of the lexer
type points struct {
	typ    pointsType
	lineno int
	text   string
...
// pointsType are the different types the lexer
// is able to parse and return.
type pointsType int

const (
	eof              pointsType = iota // end of file
...
func (it pointsType) String() string {
	if int(it) > len(stringpointsTypes) {
		return "invalid"
	}
	return stringpointsTypes[it]
}

var stringpointsTypes = []string{
	eof:              "EOF",
...
// source code and turning them in to pointss.
// pointss are interpreted by the compiler.
type lexer struct {
	input string // input contains the source code of the program

	pointss chan points // pointss is used to deliver pointss to the listener
...
// channel on which the pointss are delivered.
func Lex(name string, source []byte, debug bool) <-chan points {
	ch := make(chan points)
	l := &lexer{
		input:  string(source),
		pointss: ch,
...
// Emits a new points on to points channel for processing
func (l *lexer) emit(t pointsType) {
	points := points{t, l.lineno, l.blob()}

	if l.debug {
		fmt.Fprintf(os.Stderr, "%04d: (%-20v) %s\n", points.lineno, points.typ, points.text)
	}
