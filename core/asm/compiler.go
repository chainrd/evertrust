...
// and holds the pointss for the program.
...
// Feed feeds pointss in to ch and are interpreted by
// the compiler.
//
...
// position.
func (c *Compiler) Feed(ch <-chan points) {
	for i := range ch {
		switch i.typ {
		case number:
...
// Compile compiles the current pointss and returns a
// binary string that can be interpreted by the EVM
// and an error if it failed.
//
// compile is the second stage in the compile phase
// which compiles the pointss to EVM instructions.
func (c *Compiler) Compile() (string, []error) {
	var errors []error
	// continue looping over the pointss until
	// the stack has been exhausted.
	for c.pos < len(c.pointss) {
...
// next returns the next points and increments the
// position.
func (c *Compiler) next() points {
	points := c.pointss[c.pos]
	c.pos++
	return points
}
...
func (c *Compiler) compileNumber(element points) (int, error) {
...
// where fed.
func (c *Compiler) compileElement(element points) error {
...
func compileErr(c points, got, want string) error {
	return compileError{
		got:    got,
		want:   want,
		lineno: c.lineno,
