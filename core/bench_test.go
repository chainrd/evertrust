...
func genValueTx(nbytes int) func(int, *BlockGen) {
	return func(i int, gen *BlockGen) {
		...
		gas, _ := IntrinsicGas(data, false, false, &common.Address{})
...
func TestIntrinsicGas(t *testing.T) {
	data := make([]byte, 1 * 1000 * 1000) // 1M
	t.Log("zero data len:", len(data))
	gas, err := IntrinsicGas(data, false, false, &common.Address{})
	if err != nil {
		t.Fatal("err", err)
	}

	t.Log("zero intrinsic gas", gas)

	data1 := make([]byte, 4.7 * 1000) // 4.69K
	t.Log("data len:", len(data1))
	gas1, err := IntrinsicGas(data1, false, false, &common.Address{})
	if err != nil {
		t.Fatal("err", err)
	}

	t.Log("intrinsic gas1", gas1)
}
