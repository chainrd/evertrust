...
func TestEwasmFuns(t *testing.T) {
	code, err := ioutil.ReadFile("/app/rusthome/hello-wasm/pkg/hello_wasm_bg.wasm")
	t.Log("code", err, len(code), code)
	sentinelStart := time.Now()
	final, err := EwasmFuncs.Sentinel(code)
	t.Log("sentinel final", err, "timeused", time.Since(sentinelStart), len(final), final)
	ecode, err := EwasmFuncs.ValidateCode(final)
	t.Log("ecode", err, len(ecode), ecode)
	finalcode, err := EwasmFuncs.JoinTxdata(code, final)
	t.Log("finalcode", err, len(finalcode), finalcode)
	code, final, err = EwasmFuncs.SplitTxdata(finalcode)
	t.Log(err, len(code), len(final))
	t.Log("code", code)
	t.Log("final", final)
}
