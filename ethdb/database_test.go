...
func newTestLDB() (*ethdb.LDBDatabase, func()) {
	...
	db, err := ethdb.NewLDBDatabase(dirname, 2048, 1024)
	if err != nil {
...
func TestLDB_Put(t *testing.T) {
	...
	for i := 0; i < 1024*1024; i++ {
		MB = append(MB, byte(255))
	}
	now := time.Now()
	data := MB[:1024*1024]
	for i := 0; i < 100; i++ {
		db.Put([]byte{byte(i)}, data)
	}
	fmt.Println("db.put timeused : ", time.Since(now))
	tmp := "/tmp/trw/"
	os.MkdirAll(tmp, 0755)
	now = time.Now()
	for i := 0; i < 100; i++ {
		ioutil.WriteFile(path.Join(tmp, fmt.Sprintf("%d", i)), data, 0755)
	}
	fmt.Println("fs.write timeused : ", time.Since(now))
}
