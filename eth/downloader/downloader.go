...
import (
	...
	types2 "CRD-chain/evertrust/types"
	"sync"
	"sync/atomic"
	"time"
...
type Downloader struct {
	...
	commitCh                  chan dataPack        // [eth/63] Channel receiving inbound commit blocks CRD
	processCommitCh           chan []*types.Block  // [eth/63] Channel receiving inbound commit blocks CRD
	...
	Worker           Worker                // CRD
	sync             bool                  // CRD
	CbFetcher        *fetcher.CbFetcher    // CRD
	AssociatedCH     chan bool             // CRD
	//IsSyncNormalDeleteCommit int32                 //CRD
}
...
func (d *Downloader) Progress() ethereum.SyncProgress {
	...
	return ethereum.SyncProgress{
		...
		CurrentCommit: d.Commitchain.CurrentBlock().NumberU64(), //CRD
		HighestCommit: d.syncStatsCommitHeight,                  //CRD
	}
}
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	if height <= origin {
		log.Info("不需要进行同步,高度相同", "最新高度", height, "祖先", origin)
		return nil
	}
	fetchers := []func() error{
...
func (d *Downloader) FindAncestor(p *peerConnection, height uint64) (uint64, error) {
	...
	from := int64(head) - int64(MaxMaxHeaderFetchCRD)
	if from < 0 {
		from = 0
	}
	...
	for finished := false; !finished; {
		...
		case packet := <-d.headerCh:
			...
			for i := 0; i < len(headers); i++ {
				if number := headers[i].Number.Int64(); number != from+int64(i)*int64(engine.Cnfw.Uint64()+1) { //CRD
...
func (d *Downloader) fetchHeaders(p *peerConnection, from uint64, pivot uint64) error {
	...
	skeleton := false           // Skeleton assembly phase or finishing up //CRD true->false
...
func (d *Downloader) processCommits(p *peerConnection, wait *sync.WaitGroup, errFetchCh, errCh chan error, origin originAssertionSum, assertionSum CommitAssertionSum,
	...
	defer func() {
		log.Debug("processCommits  terminated", "processCommitCh长度", len(d.processCommitCh))
		timeOut.Stop()
		wait.Done()
		//清理processCommitCh,避免阻塞
		for empty := false; !empty; {
			select {
			case <-d.processCommitCh:
			default:
				log.Info("processCommitCh清理完毕")
				empty = true
			}
		}
	}()
	...
	for {
		...
		case <-timeOut.C:
			...
			errCh <- errors.New("processSyncCommit timeout 退出")
			return nil
...
func updateQuorum(commitBlock *types.Block, commitExtra types2.CommitExtra, consensusAddress map[common.Address]struct{}) {
	...
	case currentNum <= quorum.LessCommit || engine.PerQuorum:
		...
		if currentNum == quorum.LessCommit && !engine.PerQuorum {
			...
			quorum.VerifyUpdateQuorums.CalculateNextUpdateHeight(commitBlockHash, commitBlock, engine.UIP1)
		}
	default:
		////增加预备委员会列表
		...
		if commitBlock.Number().Cmp(quorum.VerifyUpdateQuorums.NextUpdateHeight) == 0 || commitBlock.Number().Cmp(engine.UIP1) == 0 {
			//计算下次委员会更新时间
			quorum.VerifyUpdateQuorums.CalculateNextUpdateHeight(commitBlockHash, commitBlock, engine.UIP1)
...
func (d *Downloader) VerifyQualification(block *types.Block) bool {
	//先判断收到块的作者在不在委员会中
	...
	coinbase, _, err := evertrust.SigToAddress(hash.Bytes(), header.Signature)
	if err != nil {
		log.Error("verifySignBlock SigToAddress error", "err", err)
		return false
	}

	if _, ok := consensusQuorum.Hmap[coinbase.String()]; ok {
		log.Info("同步收到的块是委员会中的成员", "收到区块的地址", coinbase.String())
		return true
	}
	...
	if land.IslandState {
		for _, add := range land.IslandQuorum {
			if add == coinbase.String() {
				log.Info("同步收到的块是分叉前委员会中的成员", "收到区块的地址", coinbase.String())
				return true
			}
		}
	}
...
func (d *Downloader) CommitSyncIsland(origin uint64) {
	d.Worker.Evertrust().Worker().CommitRw.Lock()
	defer d.Worker.Evertrust().Worker().CommitRw.Unlock() //防止processCommit先执行
	if atomic.LoadInt32(&d.AssociatedSynchronising) == 1 {
