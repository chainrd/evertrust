...
func (q *queue) DeliverBodies(id string, txLists [][]*types.Transaction, uncleLists [][]*types.Header) (int, error) {
	...
	return q.deliver("Bodies", id, q.blockTaskPool, q.blockTaskQueue, q.blockPendPool, q.blockDonePool, bodyReqTimer, len(txLists), reconstruct)
}
...
func (q *queue) DeliverReceipts(id string, receiptList [][]*types.Receipt) (int, error) {
	...
	return q.deliver("Receipts", id, q.receiptTaskPool, q.receiptTaskQueue, q.receiptPendPool, q.receiptDonePool, receiptReqTimer, len(receiptList), reconstruct)
}
...
// reason the lock is not obtained in here is because the parameters already need
// to access the queue, so they already need a lock anyway.
func (q *queue) deliver(kind string, id string, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	pendPool map[string]*fetchRequest, donePool map[common.Hash]struct{}, reqTimer metrics.Timer,
	results int, reconstruct func(header *types.Header, index int, result *fetchResult) error) (int, error) {

	// Short circuit if the data was never requested
	log.Info("查看deliverID", "id", id, "kind", kind)
	request := pendPool[id]
	if request == nil {
		for id, _ := range pendPool {
			log.Info("出错查询pendPool", "id", id)
		}
		log.Error("deliver error", "id", id)
		return 0, errNoFetchesPending
	}
	...
	for i, header := range request.Headers {
		...
		if index >= len(q.resultCache) || index < 0 || q.resultCache[index] == nil {
			log.Info("deliver数据","kind", kind,"id", id,"index",index,
				"len(q.resultCache)",len(q.resultCache),
				"q.resultOffset",q.resultOffset,"header.Number.Int64()",header.Number.Int64())
			failure = errInvalidChain
			break
		}
		if err := reconstruct(header, i, q.resultCache[index]); err != nil {
			log.Error("deliver数据!!!!!!!!!!!!!!!!!!!!", "index", q.resultCache[index].Hash, "header", header.Hash())
			failure = err
			break
		}
		hash := header.Hash()
...
func (q *queue) Prepare(offset uint64, mode SyncMode) {
	...
	log.Info("查看同步模式","模式",mode,"offset",offset)
	q.mode = mode
}
