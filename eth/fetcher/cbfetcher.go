...
func NewCBFetcher(getBlock blockRetrievalFn, verifyHeader headerVerifierFn, chainHeight chainHeightFn, insertChain chainInsertFn, dropPeer peerDropFn, normalNewHeight chainHeightFn, syncing *int32) *CbFetcher {
	...
	return &CbFetcher{
		...
		Synced:          make(chan struct{}, 10000),
		InsertCh:        make(chan struct{}, 10000),
...
func (f *CbFetcher) loop() {
	...
	for {
		...
		for !f.queue.Empty() {
			...
			//如果大于链上高度+1或者正在同步,就不进行同步
			if number > height+1 || atomic.LoadInt32(f.syncing) == 1 {
				log.Info("commit高度不合适", "commit高度", number, "当前链上高度", height)
				blockExtra,_ := types2.CommitExtraDecode(op.block)
...
func (f *CbFetcher) enqueue(peer string, block *types.Block) {
	...
	if _, ok := f.queued[hash]; !ok {
		...
		f.queues[peer] = count
		f.queued[hash] = op
		blockExtra ,_:= types2.CommitExtraDecode(block)
...
func (f *CbFetcher) insert(peer string, block *types.Block) {
	...
	go func() {
		...
		if parent == nil {
			log.Debug("Unknown parent of propagated CommitBlock", "peer", peer, "number", block.Number(), "hash", hash, "parent", block.ParentHash())
			return
		}
		...
		if block.NumberU64() <= height {
			log.Debug("Fetcher无效的Commit高度", "height", block.NumberU64(), "currentHeight", height)
			return
		}
