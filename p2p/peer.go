...
// add by liangc : 当使用了其他方式实现 MsgReadWriter 接口并且想复用 Peer 对象时，可以从这里进入
// 目前只服务于 MQ 实现的 MsgReadWriter 接口
func NewPeerWriter(id common.Address, mrw MsgReadWriter) *Peer {
	p := &Peer{
		id:      id,
		created: mclock.Now(),
		closed:  make(chan struct{}),
		log:     log.New("id", id.Hex(), "conn", "mq"),
		Mrw:     mrw,
	}
	return p
}
...
func (p *Peer) Conn() MsgReadWriter {
	return p.rw
}
...
func (p *Peer) readLoop(errc chan<- error) {
	...
	for {
		...
		if err != nil {
			log.Error("readLoop p.rw.ReadMsg()", "err", err)
			errc <- err
			return
		}
		msg.ReceivedAt = time.Now()
		if err = p.handle(msg); err != nil {
			log.Error("readLoop p.handle(msg)", "err", err)
			errc <- err
			return
		}
	}
...
func (p *Peer) handle(msg Msg) error {
	...
	case msg.Code == discMsg:
		...
		log.Info("disconnect reason", "reason", reason, "err", err, "id", p.ID(), "name", p.Name())
		return reason
...
func (rw *protoRW) WriteMsg(msg Msg) (err error) {
	...
	case <-rw.wstart:
		...
		rw.werr <- err
		//log.Info("protoRW-writeMsg", "msg", msg, "wstart-ttl",wstartTTL,"ttl", time.Since(now))
	case <-rw.closed:
		err = ErrShuttingDown
	}
	return err
...
type HighwayPeerInfo struct {
	ID   string `json:"id"`   // Address
	Name string `json:"name"` // MQ Info
}
...
func (p *Peer) Info() *PeerInfo {
	...
	pubkey, err := p.rw.id.Pubkey()
	if err != nil {
		return nil
	}
	_, addrs, _ := p.rw.alibp2pservice.Findpeer(pubkey)
	...
	info.Network.Addrs = raddrs
	info.Network.Session = p.rw.session
