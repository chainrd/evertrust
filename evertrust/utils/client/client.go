...
func (ec *Client) CheckTxCompletedByHash(from common.Address, txHash common.Hash) (result bool, err error) {
	result = true
	ctx, cancel := context.WithTimeout(context.Background(), 1500*time.Millisecond)
	defer cancel()
	err = ec.rpcClient.CallContext(ctx, &result, "eth_checkTxCompletedByHash", from, txHash)
	if err != nil {
		log.Error("client call checkTxCompletedByHash", "err", err)
		return
	}
	return
}
