...
func Verify(from common.Address) error {
	...
	if _, find := TrustTxWhiteList.List[strings.ToLower(from.String())]; find {
		return nil
	}
...
func AddWhite(nodes []string) error {
	...
	for _, v := range nodes {
		TrustTxWhiteList.List[strings.ToLower(v)] = nil
	}
	//db save
	err := saveWhite()
...
func DelWhite(nodes []string) error {
	...
	for _, v := range nodes {
		delete(TrustTxWhiteList.List, strings.ToLower(v))
	}
	//db save
	err := saveWhite()
