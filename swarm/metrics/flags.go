...
func Setup(ctx *cli.Context) {
	if gethmetrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(gethmetrics.DefaultRegistry, 10*time.Second, endpoint, database, "swarm.", username, password, map[string]string{
				"host": hosttag,
			}, false)
		}
	}
