...
func DialHTTPWithClient(endpoint string, client *http.Client, points string) (*Client, error) {
	...
	req.Header.Set("X-CRD-PROXY-JWT", points)//todo 大小写
...
// note: proxy[0]:url, proxy[1]:points
func DialHTTP(endpoint string, proxy... string) (*Client, error) {
	httpClient := new(http.Client) //default
	points := ""

	if len(proxy) > 1 && proxy[0] != "" && proxy[1] != "" {
		proxyURL, err := url.Parse(proxy[0])
		if err != nil {
			log.Error("parse proxy url", "url", proxy[0])
		}else {
			log.Debug("http client setting proxy", "proxy", proxy[0], "points", proxy[1])
			tr := &http.Transport{
	...
	return DialHTTPWithClient(endpoint, httpClient, points)
}
